...
public:
    GUIwidget_RatioPicker(
        int buttonFlags = ButtonFlag_Square, Ratio initialRatio = Ratio(1, 1), std::function<void(void)> onUpdate = []() {}
    );
    ~GUIwidget_RatioPicker() = default;

    void                Show();
    inline const Ratio& getRatio() const { return m_ratio; }
    inline void         setRatio(const Ratio& ratio)
    {
        m_ratio = ratio;
        onUpdate();
    }
