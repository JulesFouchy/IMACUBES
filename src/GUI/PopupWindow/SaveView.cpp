...
PopupWindow_SaveView::PopupWindow_SaveView()
    : PopupWindow_WithConfirmationWarning("Saving current view"), m_widthHeightRatioPicker(), m_filepathPicker(" PNG (*.png)\0*.png;*.PNG\0All Files (*.*)\0*.*\0"), m_nbSamplesForMSAA(4)
{
...
void PopupWindow_SaveView::Show()
{
    ...
    ImGui::SameLine();
    ImGui::HelpMarker("Number of samples used by Multi Sampling to reduce aliasing\n Note : MSAA is currently broken because we use Deferred Rendering");
    EndWindow();
}

void PopupWindow_SaveView::OnConfirmation()
{
    Locate::renderer().save(m_widthHeightRatioPicker.getWidth(), m_widthHeightRatioPicker.getHeight(), m_filepathPicker.getFilepath(), m_nbSamplesForMSAA);
}
