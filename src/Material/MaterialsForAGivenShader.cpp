...
MaterialsForAGivenShader::MaterialsForAGivenShader(const std::string& vertexFilepath, const std::string& fragmentFilepath, int shaderIndex)
    : m_shaderLID(Locate::shaderLibrary().LoadShader(vertexFilepath, fragmentFilepath, false)), m_cubes(), m_name(MyString::RemoveFileExtension(MyString::RemoveFolderHierarchy(fragmentFilepath))), m_shaderIndex(shaderIndex)
{
...
void MaterialsForAGivenShader::parseShaderAndCreateUniformDescriptions(const std::string& fragmentFilepath)
{
    ...
    if (file.is_open())
    {
        ...
        while (std::getline(file, line))
        {
            ...
            if (line.find("struct MaterialParameters") != std::string::npos)
            { // TODO check that the line isn't commented out
