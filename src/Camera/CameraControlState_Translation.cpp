...
CameraControlState_Translation::CameraControlState_Translation(Camera* camera)
	...
	m_initialTranslation(camera->getLookedAtPoint())
{
}
...
void CameraControlState_Translation::update(float dt) {
	...
	m_camera->setLookedAtPoint(m_initialTranslation + (m_camera->getLocalX()*mouseDL.x +m_camera->getLocalY()*mouseDL.y)* Settings::CAMERA_TRANSLATION_SPEED_PER_INCH * m_camera->m_sphereCoord.getRadius());
