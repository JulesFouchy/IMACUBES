...
DirectionalLight::DirectionalLight(int dirLightIndex, float angleUp, float angleGround, const glm::vec3& color, float intensity, const std::string& name)
    : Light(name, color, intensity), m_dirLightIndex(dirLightIndex), m_direction("", HistoryType::Lights, SphericalCoordinates_AngularPart(angleUp, angleGround)), m_bCastShadows(true)
{
