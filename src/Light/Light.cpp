...
void Light::setUniforms(const std::string & uniformName, UniformUpdateList & uniformsList) {
	uniformsList.setUniform(uniformName + ".color", m_color.value());
	uniformsList.setUniform(uniformName + ".intensity", m_intensity.value());
}

void Light::ImGui_Sliders() {
	m_color.ImGui_Slider();
	m_intensity.ImGui_Drag(0.01f);
	if (m_intensity.value() < 0.0f)
		m_intensity.value() = 0.0f;
}