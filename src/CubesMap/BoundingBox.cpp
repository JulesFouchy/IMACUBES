...
BoundingBox::BoundingBox()
    : m_minCorner(Locate::cubesMap().minValidX(), Locate::cubesMap().minValidY(), Locate::cubesMap().minValidZ()), m_maxCorner(Locate::cubesMap().maxValidX(), Locate::cubesMap().maxValidY(), Locate::cubesMap().maxValidZ())
{
    computeInfos();
}
...
BoundingBox::BoundingBox(const glm::ivec3& v0, const glm::ivec3& v1, BboxGenerationMode mode)
{
    ...
    case CORNERS:
        // v0 = corner0
        // v1 = corner1
        m_minCorner = glm::ivec3(std::min(v0.x, v1.x), std::min(v0.y, v1.y), std::min(v0.z, v1.z));
        m_maxCorner = glm::ivec3(std::max(v0.x, v1.x), std::max(v0.y, v1.y), std::max(v0.z, v1.z));
        break;
    default:
        break;
    }
