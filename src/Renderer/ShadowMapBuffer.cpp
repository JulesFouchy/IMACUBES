...
ShadowMapBuffer::ShadowMapBuffer()
    : m_shadowMap(GL_DEPTH_COMPONENT, GL_DEPTH_COMPONENT, GL_FLOAT, GL_NEAREST, GL_CLAMP_TO_BORDER), m_resolution("Resolution", HistoryType::Lights, 2323, 1, 8192), m_minBias("Min Bias", HistoryType::Lights, 0.001f, 0.0f, 0.03f), m_maxBias("Max Bias", HistoryType::Lights, 0.008f, 0.0f, 0.03f), m_nearPlane("Near plane", HistoryType::Lights, 0.1f, 0.1f, 2.0f), m_farPlane("Far plane", HistoryType::Lights, 72.0f, 5.0f, 100.0f), m_cropFactor("Crop factor", HistoryType::Lights, 69.0f, 1.0f, 100.0f), m_lightDistance("Light distance", HistoryType::Lights, 40.0f, 0.0f, 100.0f)
{
...
ShadowMapBuffer::ShadowMapBuffer(ShadowMapBuffer&& other) noexcept
    : m_frameBufferID(other.m_frameBufferID), m_shadowMap(std::move(other.m_shadowMap)), m_minBias(other.m_minBias), m_maxBias(other.m_maxBias), m_resolution(other.m_resolution), m_nearPlane(other.m_nearPlane), m_farPlane(other.m_farPlane), m_cropFactor(other.m_cropFactor), m_lightDistance(other.m_lightDistance), m_lightViewMat(other.m_lightViewMat), m_lightProjMat(other.m_lightProjMat), m_lightVPMat(other.m_lightVPMat)
...
{
    m_lightViewMat = glm::lookAt(-lightDir * m_lightDistance.getValue(), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
}
