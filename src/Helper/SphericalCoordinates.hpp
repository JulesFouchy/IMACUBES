...
#include <glm/glm.hpp>
#include "SphericalCoordinates_AngularPart.hpp"

struct SphericalCoordinates {
    SphericalCoordinates(float radius = 1.0f, float angleGround = 0.0f, float angleUp = 0.0f)
        : m_radius(radius), m_angles(angleGround, angleUp), m_xyz(0.0f), m_bMustRecomputeXYZ(true)
    {}
    ...
    inline float& angleGround()
    {
        m_bMustRecomputeXYZ = true;
        return m_angles.angleGround();
    }
    inline float& angleUp()
    {
        m_bMustRecomputeXYZ = true;
        return m_angles.angleUp();
    }
    ...
    inline const glm::vec3 getXYZ()
    {
        if (m_bMustRecomputeXYZ)
            computeXYZ();
        return m_xyz;
    }
