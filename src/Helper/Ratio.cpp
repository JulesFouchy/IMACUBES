...
Ratio::Ratio(unsigned int numerator, unsigned int denominator)
    : m_numerator(numerator), m_denominator(denominator), m_bMustRecomputeAspectRatio(false), m_bMustRecomputeNumAndDenom(false)
{
    recomputeAspectRatio();
}

Ratio::Ratio(float fRatio)
    : m_ratio(fRatio), m_bMustRecomputeAspectRatio(false), m_bMustRecomputeNumAndDenom(false)
{
...
void Ratio::recomputeNumAndDenom() const
{ // we need to mark this method as const so that getNumerator() etc. can be marked as const, and therefore be used when a ratio is passed by const reference
