...
Shader::Shader(const std::string& vertexShaderFilepath, const std::string& fragmentShaderFilepath, bool compileShader, const std::vector<std::string>& lookForInFS, const std::vector<std::string>& replaceWithInFS, const std::vector<std::string>& lookForInVS, const std::vector<std::string>& replaceWithInVS)
    : m_shaderId(-1), m_vsID(-1), m_fsID(-1), m_vertexShaderFilepath(MyFile::GetFullPath(vertexShaderFilepath)), m_fragmentShaderFilepath(MyFile::GetFullPath(fragmentShaderFilepath)), m_bCreatedSuccessfully(true)
{
    if (compileShader)
    {
...
Shader::Shader(Shader&& other) noexcept
    : m_shaderId(other.m_shaderId), m_vsID(other.m_vsID), m_fsID(other.m_fsID), m_vertexShaderFilepath(other.m_vertexShaderFilepath), m_fragmentShaderFilepath(other.m_fragmentShaderFilepath), m_bCreatedSuccessfully(true)
{
    other.m_shaderId = -1;
}
