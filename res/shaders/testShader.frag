...
struct MaterialParameters{
	vec3 u_Offset; // min -2 -2 -2 max 2 2 2
	float u_starIntensity; // default 1.058 min 0 max 2
	float u_starIntensityPow; // default 50 min 0 max 40
	float u_noiseScale; // default 300 min 0 max 500
	float u_distortInScale; // default 0 min 0 max 60
	float u_distortOutScale; // default 100 min 0 max 60
	vec3 u_SkyColor; // default 0.004f, 0.008f, 0.070f
	vec3 u_StarColor; // default 1 0.969 0.753
};
...
void main() {
	...
	//float t = snoise(params[0].u_noiseScale*uv);
	//color = mix(params[0].u_SkyColor, params[0].u_StarColor, t);
